...
| Header B-OOM pulse | `(tabs)/index.tsx` `HeaderSpark`: old 30s single glide → subtle **heartbeat pulse** (appear→scale≤1.10 (~0.25mm, under 1mm cap)→shrink→fade, ~2.6s). scale+opacity GPU-only; `overflow:hidden` clips it in the header gap → never touches logo/search/assistant. Removed unused `boxW`/`onLayout`/`LayoutChangeEvent` | `e19ee58` | typecheck 0 · guards 75/75 · videoReview: subtle, centered, no collision/layout-shift |
